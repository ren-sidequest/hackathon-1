...
API4 使用右下角可收起的演示角色入口，保留两端独立目录、侧边栏和现有业务合同。开发双端地址、导航记忆、草稿保护和 T61–T66 验证见 [演示角色切换](docs/DEMO_ROLE_SWITCH.md)。入口不是账号登录／权限切换；不得携带私人笔记或自动触发业务写入。继续使用现有 `npm run test:api4 --prefix app/candidate` 检查，无新增依赖。
...
- 开展产品功能、页面、交互、演示数据或验收相关工作前，先阅读 [产品蓝图](docs/product/EvidenceBridge_PRODUCT_BLUEPRINT.md) 和 [UI 与交互基线](docs/product/EvidenceBridge_BASELINE.md)。视觉工作以用户已验收的黑金／白金页面和当前服务器业务为准；2026-09-19 用户明确两张原始概念图不再作为后续修改参考。
...
- 图 1 是候选人核心工作台参考，图 2 是双端页面与流程参考。已知差异按 Baseline 落实：侧栏随日夜主题变化、双端统一黑金／白金强调色，默认夜间；候选人核心输入采用调查板，不照搬图 2 的单一大文本框。图片外围注释不作为产品界面内容；图本身不新增后端要求；当前共享 API 与有限两版范围来自用户明确确认的修订 3。其他实质冲突先指出具体位置并确认。
