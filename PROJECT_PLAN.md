...
- 阶段：HR 端已实现独立可运行的前端演示；Candidate 与跨端同步仍待实现。
...
| --- | --- | --- | --- |
| 候选人申请与 HR 初始证据报告 | 必须 | 可加载演示材料，显示 SQL、Data Analysis 为 Supported，Business Problem Solving 为 Uncertain | HR 报告已实现；Candidate 申请待实现 |
...
| 项目 | 当前决定 |
| --- | --- |
| 应用形态与技术栈 | 浏览器 Web 应用，桌面演示优先；HR 独立使用 React 19 + Vite 7，Candidate 技术栈由另一端确认 |
| 运行时、包管理器及版本 | HR：Node.js 22.12+、npm，版本与依赖见 app/hr/package.json 和 package-lock.json |
| 模块划分与代码目录 | HR 与 Candidate 分别设计实现；已建立 [app/hr/](app/hr/README.md) 与 [app/candidate/](app/candidate/README.md)，HR 已实现，Candidate 保留初始化说明；共享模块目录与文件归属待约定 |
| 接口、数据结构与存储方式 | HR 自有静态数据与浏览器 localStorage；不定义跨端接口。跨端任务、提交、证据、审核状态与重置合同仍待双方约定 |
| 外部服务及离线／模拟方案 | 允许静态 JSON、本地状态和预生成输出；是否引入真实服务待确认，不默认需要生产后端 |
| 本地运行方式和演示设备 | HR：npm run dev / build / preview（--prefix app/hr），127.0.0.1:5186；本机浏览器桌面演示 |
...
- 审核分支：Needs More Evidence 与 Evidence Still Insufficient 不应误显示为已验证；HR 本地保留 Uncertain 与必填审核备注；可重新打开审核。跨端回流仍待约定。
- 错误与边界情况：至少覆盖空调查内容提交、重复提交、刷新后的状态以及演示重置；HR 已覆盖空任务、重复发送/加载、刷新恢复与演示重置；Candidate 的空调查内容与提交边界由另一端实现。
- 演示准备：统一使用文档中的 HarbourCart 场景（流量 +18%、转化率 3.4% → 2.6%、广告支出 +15%）；HR 操作脚本见 app/hr/README.md；侧栏提供 Reset demo。
