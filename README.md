...
`ren-sidequest` 的黑客松项目 **EvidenceBridge**：找出候选人材料中的能力证据缺口，通过定向工作样本任务补充证据，供 HR 人工审核。MVP 是前端演示优先的浏览器 Web 应用；产品文档与概念图已接入，技术栈待确定，尚无应用代码。
...
HR 与 Candidate 由两位协作者分别设计和实现，各自分支、各自 PR。已建立 [app/hr/](app/hr/README.md) 与 [app/candidate/](app/candidate/README.md) 两个开发目录，各含初始化说明；两端共享产品基线，并在并行实现前约定技术栈、跨端数据和状态。当前没有应用代码，不预设前端框架。
...
这些命令仅检查协作及产品文档、本地链接（含概念图路径）和工作区空白错误；当前 Actions 也只做仓库基础检查。**通过不代表图片内容正确或应用测试、构建、演示、部署成功。**
...
以两份产品文档为依据，确定技术栈、两端共享的数据与状态约定、演示重置方式，再分别开展 HR 和 Candidate 实现。不要复制其他项目的业务、运行目录、凭据或部署配置。
