...
const output=new URL('../../../docs/backend/r6/',import.meta.url);
const exampleDir=new URL('examples/',output);
const token='generated-local-example-token-only';
const apps=[]; const files=new Map();
const manifest={schemaVersion:'4.0',generatedAt:new Date().toISOString(),kind:'executed-in-memory-synthetic-fixtures',modelCalls:0,
  scope:'Owned four-candidate API 4.0 requests. Random IDs, hashes and times belong to this run; read fresh state in a running service. Preset annotations are AI-authored fixtures, not completed human calibration.',requests:[]};
...
try{
 ...
 const initial={};for(const id of ['amy-chen','ann-li','david-liu','jamie-parker'])initial[id]=await request(`initial-${id}`,`/api/demo?candidateId=${id}`);
 await request('error-missing-candidate','/api/demo',undefined,400);
 await request('error-old-candidate','/api/demo?candidateId=alex-chen',undefined,409);
 await request('error-old-contract','/api/demo/task/send',{schemaVersion:'3.0'},409);
 let alex=initial['amy-chen'];
 ...
 for(const [id,target] of [['amy-chen','business-problem-solving'],['ann-li','sql'],['jamie-parker','data-analysis']]){
  let d=id==='amy-chen'?alex:initial[id];const template=d.taskTemplates[target];
 ...
 await request('admin-reset','/api/demo/reset',{schemaVersion:'4.0',sessionId:alex.sessionId},200,true);
 const disabled=await scenario('disabled');let d=await disabled.request('disabled-initial','/api/demo?candidateId=david-liu');const t=d.taskTemplates.sql;
 ...
 await disabled.request('analysis-failed-state','/api/demo?candidateId=david-liu');
 ...
 console.log(`Executed ${manifest.requests.length} API4 requests; generated ${files.size} example files. No model calls or formal database changes.`);
