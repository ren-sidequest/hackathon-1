...
export async function runReset({ base = baseUrl(), token = process.env.DEMO_ADMIN_TOKEN ?? '' } = {}) {
  ...
  if (!['2.0', '4.0'].includes(version)) throw Object.assign(new Error('Select a supported demo service.'), { code: 'UNSUPPORTED_CONTRACT' });
  const readPath = version === '4.0' ? '/api/demo/comparison' : '/api/demo';
