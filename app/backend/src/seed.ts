...
export function createSeed() {
  ...
  const context = `# HarbourCart · Business context

Synthetic demonstration data only. Two consecutive, non-overlapping four-week windows use the same session and order definitions. No customer-level data is included.
...
Investigate the strongest signals, separate observations from hypotheses, request discriminating evidence, and recommend a measured next step. No single prescribed conclusion or hiring decision is implied.`;
  const resources = [
    ...
    resource('campaigns.csv', 'Derived acquisition-channel comparison, not individual campaign-level data.',
    ...
    resource('landing_pages.csv', 'Current Paid Search device partition; page identity, history and speed are not provided.',
  ...
  const sources = [
    { id: 'customer_churn_analysis.sql', name: 'customer_churn_analysis.sql', kind: 'sql_example', provenance: 'synthetic',
      content: `-- Prepared sample; inspected as source code, not claimed to have been executed.
...
FROM monthly_orders;` },
    { id: 'sales_analysis_project.md', name: 'sales_analysis_project.md', kind: 'calculation_table', provenance: 'synthetic',
      content: `# Prepared sales calculation sample

This earlier application project is distinct from the targeted task dataset. Integer monthly totals support the calculations below; no business recommendation was supplied.

| Month | Orders | Revenue AUD | Average order value AUD | Revenue change |
| --- | ---: | ---: | ---: | ---: |
| April | 400 | 12000 | 30 | — |
| May | 450 | 13500 | 30 | 12.50% |
| June | 500 | 15000 | 30 | 11.11% |

Average order value = revenue / orders. Revenue change = (current revenue - previous revenue) / previous revenue. Percentages shown to two decimals. These calculations show descriptive comparison; they do not establish why revenue changed.` },
    { id: 'alex_chen_resume.md', name: 'alex_chen_resume.md', kind: 'self_statement', provenance: 'synthetic',
      content: 'Alex Chen — Junior analyst with SQL, Python and dashboard experience. Projects include customer churn analysis and an e-commerce sales dashboard. Presented analytical findings in university team projects. This is a candidate self-statement, not independent verification of problem-solving behaviour.' },
  ];
  const requirements = [
    { id: 'sql', title: 'SQL', statement: 'Write SQL queries to explore, aggregate and analyse business data.' },
    { id: 'data-analysis', title: 'Data Analysis', statement: 'Analyse and compare data using reproducible calculations.' },
    { id: 'business-problem-solving', title: 'Business Problem Solving', statement: 'Frame business problems, test hypotheses and recommend evidence-led actions.' },
  ];
  const initialReport = [
    { requirementId: 'sql', status: 'supported', displayStatus: 'Supported', mode: 'preset',
      summary: 'The supplied SQL sample uses aggregation, a CTE and a window function.',
      sourceRefs: [{ sourceId: sources[0]!.id, locator: 'lines 2–9', quote: 'LAG(revenue) OVER (PARTITION BY customer_id ORDER BY month) AS previous_month_revenue' }],
      uncertainty: 'Source-code inspection only; execution correctness and production-scale performance have not been observed.' },
    { requirementId: 'data-analysis', status: 'supported', displayStatus: 'Supported', mode: 'preset',
      summary: 'The prepared calculation table correctly compares monthly totals and percentage change.',
      sourceRefs: [{ sourceId: sources[1]!.id, locator: 'table / May', quote: '| May | 450 | 13500 | 30 | 12.50% |' }],
      uncertainty: 'The supplied totals are prepared data; cleaning, independent reproduction and ambiguous business decisions remain unobserved.' },
    { requirementId: 'business-problem-solving', status: 'uncertain', displayStatus: 'Uncertain', mode: 'preset',
      summary: 'Technical samples and a self-statement are present; a reasoned business recommendation is not yet observable.',
      sourceRefs: [{ sourceId: sources[2]!.id, locator: 'paragraph 1', quote: 'Presented analytical findings in university team projects.' }],
  ...
  return {
    ...
    task: { id: 'conversion-drop-investigation', requirementId: 'business-problem-solving', title: 'Conversion Drop Investigation',
      instructions: 'Investigate what is driving the conversion decline. Use the supplied data to identify the strongest signals, form testable hypotheses, and recommend a prioritised next step. Distinguish what the data shows from what still needs to be validated.',
      timeboxMinutes: 20, timeboxEnforced: false, mode: 'preset', resourceIds: resources.map(r => r.id) },
