...
async function serve(t, version) {
  const app = await (version === '3.0' ? createRevision5App : createApp)({ databasePath: ':memory:', adminToken: token });
...
test('reset CLI detects API3 and resets all four owned cases through real HTTP', async t=>{
  const base=await serve(t,'3.0'), before=await get(base,'/api/demo/comparison');
  for(const row of before.candidates) {
    const d=await get(base,`/api/demo?candidateId=${row.candidate.id}`);
    await post(base,'/api/demo/task/send',{schemaVersion:'3.0',sessionId:d.sessionId,candidateId:d.candidate.id,jobId:d.job.id,datasetVersion:d.datasetVersion,taskId:d.task.taskId,targetRequirementId:'sql',templateId:'harbourcart-sql-v1',instructions:'Synthetic reset test task',gapReason:'Synthetic source gap'});
  }
  const result=await invoke(base);const output=JSON.parse(result.stdout);
  assert.deepEqual(output,{status:'reset',schemaVersion:'3.0',candidatesReset:4,taskStatus:'draft',oldReferencesInvalidated:true});assert.ok(!result.stdout.includes(token));
...
test('invalid token leaves the case untouched and credentials out of logs',async t=>{
  const base=await serve(t,'3.0'), before=await get(base,'/api/demo/comparison'), wrong='WRONG-SYNTHETIC-reset-cli-admin-token';
...
test('unknown service contract stops before any reset write',async t=>{
  ...
  const server=createServer((req,res)=>{if(req.method==='POST')writes++;res.setHeader('Content-Type','application/json');res.end(JSON.stringify({status:'ok',storage:'sqlite',schemaVersion:'4.0'}));});
