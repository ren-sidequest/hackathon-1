...
test('actual V1 → More → V2 → Confirm, immutable histories, quotations and private notes', async ({page:candidate,context,request},info) => {
  ...
  await expect(hr.getByText(first,{exact:true})).toBeVisible();
  ...
  await expect(hr.locator('dialog mark')).toContainText('😀中文');
  await hr.getByRole('button',{name:'Close dialog'}).click();
  ...
  await hr.goto(`${hrUrl}/#review`); await refresh(hr);
  await hr.getByRole('tab',{name:'Work sample',exact:true}).click();
  await expect(hr.getByText(second,{exact:true})).toBeVisible();
  ...
  await hr.getByRole('tab',{name:'Work sample',exact:true}).click();
  await expect(hr.getByText(first,{exact:true})).toBeVisible();
  await hr.getByRole('tab',{name:'AI observations',exact:true}).click();
  await hr.locator('.eb-citation').first().click();await expect(hr.locator('dialog mark')).toContainText('EB-UI-V1');
  await hr.getByRole('button',{name:'Close dialog'}).click();
  await hr.getByLabel('Submission version',{exact:true}).selectOption('2');
  await hr.getByRole('button',{name:'Analyze current submission',exact:true}).click();
  await expect(hr.getByText('Manual rules simulation · not a model run',{exact:true})).toBeVisible();
  await hr.locator('.eb-citation').first().click();await expect(hr.locator('dialog mark')).toContainText('EB-UI-V2');await hr.getByRole('button',{name:'Close dialog'}).click();
...
test('review closes in-flight analysis; late result does not reopen V1 or leak into V2',async({page:candidate,context,request})=>{
  ...
  await hr.goto(`${hrUrl}/#review`); await refresh(hr);await hr.getByRole('tab',{name:'AI observations'}).click();await expect(hr.getByText(/AI_REVIEW_CLOSED/).last()).toBeVisible();await expect(hr.getByRole('button',{name:'Start new analysis attempt'})).toHaveCount(0);
});
...
test('API dataset, downloads, theme/sidebar and narrow-screen navigation',async({page:candidate,context},info)=>{
  const hr=await context.newPage();await startPair(candidate,hr,'Data check');await candidate.getByRole('button',{name:'Back to investigation'}).click();await expect(candidate.getByText('1,180,000',{exact:true})).toBeVisible();await expect(candidate.getByText('30,680',{exact:true})).toBeVisible();await expect(candidate.getByText('426,000',{exact:true})).toBeVisible();await expect(candidate.getByText('31,200',{exact:true})).toHaveCount(0);
