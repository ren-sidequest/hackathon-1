...
export function AssessmentPanel({ data, controller, initialStage = 'application_review', onEditingChange, onTask, openTask, evidenceFocus }: { data: Demo; controller: Api3Controller; initialStage?: Stage; onEditingChange?: (editing: boolean) => void; onTask?:(intent:TaskIntent)=>void; openTask?:()=>void; evidenceFocus?:EvidenceFocus }) {
  ...
  return <>
    <section className="eb-panel r5-assessment-header" aria-label="Assessment context"><div className="eb-heading"><h2>Evidence before a number</h2><button className="eb-action" onClick={() => download(`${data.candidate.id}-${stage}-assessment-${record?.assessmentRevision ?? "unassessed"}.md`, assessmentReport(data, stage, record), "text/markdown")}>Export assessment report</button><button className="eb-action primary eb-star-border" disabled={controller.busy || !!controller.pending || historical || !currentWork} onClick={() => { setEdit(true); onEditingChange?.(true); }}><StarTrail />Edit human assessment</button></div>
