...
export function AssessmentPanel({ data, controller, initialStage = 'application_review', onEditingChange }: { data: Demo; controller: Api3Controller; initialStage?: Stage; onEditingChange?: (editing: boolean) => void }) {
  ...
  return <>
    ...
    <div className="r5-assessment-grid"><nav className="r5-evidence-rail" aria-label="Evidence criteria"><div className="r5-evidence-nav-heading"><h2>Assessment criteria</h2><small>{activeCriteria.length} public standards · Select one to inspect its evidence</small></div><div className="r5-evidence-options">{activeCriteria.map(c => {
      ...
      return <button key={c.id} className="r5-criterion-toggle" title={`${c.id} · ${c.title}`} aria-label={`${c.id} · ${c.title}`} aria-expanded={activeId === c.id} onClick={() => { setExpanded(c.id); if (ref) setCitation({sourceId:ref.sourceId,quote:ref,reused:!!reused}); else setCitation(null); }}><span className="r5-criterion-id">{c.id}</span><span><strong>{c.title}</strong><small className="r5-mark">{!entry ? 'Not assessed' : entry.mark === 'NE' ? 'NE' : `${entry.mark}/4`} · {c.requirementId === 'sql' ? 'SQL' : c.requirementId === 'data-analysis' ? 'DA' : 'BPS'}</small></span></button>;
    })}</div></nav><aside className="r5-original-pane"><section className="eb-panel"><label className="eb-field">Original material<GlideSelect ariaLabel="Review source" value={source?.sourceId ?? ''} onChange={value => setCitation({ sourceId: value, reused: citation?.reused })} options={quotedContext.sources.map(s => ({value:s.sourceId,label:`${s.sourceId} · ${s.location}`}))}/></label></section><InlineSource name={data.candidate.name} candidateId={data.candidate.id} context={quotedContext} source={source} quote={citation?.quote} revealRequest={citation?.revealRequest}/><details className="eb-panel"><summary>Three separate decisions</summary><p><strong>Assess:</strong> judge individual standards.</p><p><strong>Review evidence:</strong> confirm or request a bounded revision.</p><p><strong>Retain:</strong> choose who to discuss further.</p><p>None of these automatically performs the other two.</p></details></aside><section aria-label="Assessment criteria">{activeCriteria.filter(c => c.id === activeId).map(c => {
