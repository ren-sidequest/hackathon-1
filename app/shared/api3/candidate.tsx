...
function CandidateSession({ data, controller, page, go }: Props) {
  ...
  return <>
    ...
    {page === 'application' && <><Heading eyebrow={`${data.company.name} · ${data.job.title}`} title="Your application materials"/><div className="r5-two"><SpotlightCard className="eb-panel"><h2>{candidate.name}</h2><p>{candidate.background}</p><p>Preset synthetic samples. These preset materials are not newly uploaded files. Demo identity selection is separate from editing access.</p>{data.application.sources.map(s => <button className="r5-source-card" key={s.sourceId} onClick={() => setSource(s)}><strong>{s.sourceId}</strong><small>View this person’s preset material ↗</small></button>)}</SpotlightCard><StarBorder className="eb-panel"><h2>Only supplement what is needed</h2><p>{sent ? `A ${skill} task has been requested.` : 'No supplementary task has been requested. Your application can be compared using the available evidence.'}</p><p>Receiving a task is not rejection. Having no task is not an offer.</p><button className="eb-action primary" onClick={() => go('tasks')}>View task status</button></StarBorder></div></>}
