import type { AssessmentRecord, CandidateId, Comparison, Demo, Envelope, Stage } from '../api3-types';

export const candidateIds: CandidateId[] = ['alex-chen', 'maya-patel', 'leo-zhang', 'sam-taylor'];
const writePaths = ['/task/send', '/submission', '/analysis', '/review', '/assessment', '/shortlist'];
export class Api3Error extends Error {
...
export function baseBinding(data: Demo) {
  if (data.job.id !== 'junior-data-analyst') throw new Api3Error('JOB_MISMATCH', 'This contract supports the current Junior Data Analyst role only.');
...
export function taskBinding(data: Demo) {
  if (!data.task.targetRequirementId) throw new Api3Error('TASK_NOT_SENT', 'Refresh and wait for HR to send a targeted task.');
...
export function submissionBinding(data: Demo) {
  if (!data.submission) throw new Api3Error('NO_SUBMISSION', 'Select a current submitted work sample.');
...
export class Api3Client {
  ...
  constructor(public base: string, private storageKey: string, private fetcher: typeof fetch = (...args) => fetch(...args)) {
    try {
      ...
      if (p && writePaths.includes(p.path) && /^[a-zA-Z0-9_-]{8,100}$/.test(p.key) && p.body?.schemaVersion === '3.0' && typeof p.body.sessionId === 'string' && candidateIds.includes(p.body.candidateId) && new TextEncoder().encode(JSON.stringify(p.body)).length <= 128 * 1024) this.pending = p;
  ...
  reconcile(data: Demo) {
    ...
    if (!p) return;
    if (p.body.sessionId !== data.sessionId) { this.save(null); return; }
  ...
  private async request(path: string, init?: RequestInit): Promise<{ data: Demo | Comparison; meta: { replayed: boolean } }> {
    ...
    if (!response.ok) {
      ...
      if (typeof error?.code === 'string' && typeof error?.message === 'string') throw new Api3Error(error.code, error.message, typeof error.requestId === 'string' ? error.requestId : '', Boolean(init) && response.status >= 500 && error.code === 'INTERNAL_ERROR', response.status);
      throw new Api3Error('INVALID_RESPONSE', 'Unexpected service response. Keep the original receipt for retry.', '', Boolean(init), response.status);
    }
    const d = result?.data;
    if (d?.schemaVersion !== '3.0' || typeof d.sessionId !== 'string' || !Number.isInteger(d.revision) || !d.rubric || typeof result?.meta?.replayed !== 'boolean') throw new Api3Error('SCHEMA_MISMATCH', 'This mode needs EvidenceBridge API3. Choose the matching backend; there is no automatic mock fallback.', '', Boolean(init));
    if (path === '/comparison') {
      if (!Array.isArray(d.candidates) || d.candidates.length !== 4 || new Set(d.candidates.map((row: { candidate?: { id?: string } }) => row.candidate?.id)).size !== 4 || d.candidates.some((row: { candidate?: { id?: CandidateId } }) => !row.candidate?.id || !candidateIds.includes(row.candidate.id))) throw new Api3Error('SCHEMA_MISMATCH', 'The comparison response has an invalid candidate set.');
    } else if (!candidateIds.includes(d.candidate?.id) || !Array.isArray(d.versions) || !d.workflow || !d.assessment || !d.shortlist || !Array.isArray(d.dataset?.resources)) throw new Api3Error('SCHEMA_MISMATCH', 'The selected case does not match API3.', '', Boolean(init));
    return result;
  }
  async read(candidate: CandidateId): Promise<Demo> {
    const { data } = await this.request(`?candidateId=${encodeURIComponent(candidate)}`);
    if (!('candidate' in data) || data.candidate.id !== candidate) throw new Api3Error('IDENTITY_MISMATCH', 'The response belongs to a different candidate. It was not displayed.');
    return data;
  }
  ...
  async write(path: string, body: Record<string, unknown>): Promise<Envelope> {
    if (this.pending || this.running) throw new Api3Error('PENDING_ACTION', 'Resolve the original receipt before starting a new write.');
    if (!writePaths.includes(path) || body.schemaVersion !== '3.0' || !candidateIds.includes(body.candidateId as CandidateId)) throw new Api3Error('INVALID_REQUEST', 'Use an explicit API3 candidate binding.');
    if (new TextEncoder().encode(JSON.stringify(body)).length > 128 * 1024) throw new Api3Error('PAYLOAD_TOO_LARGE', 'The request exceeds 128 KiB. Shorten the public text.');
    this.save({ path, body: structuredClone(body), key: crypto.randomUUID() });
    return this.retry();
  }
  async retry(): Promise<Envelope> {
    if (this.running) throw new Api3Error('PENDING_ACTION', 'The original action is still running.');
    const p = this.pending;
    if (!p) throw new Api3Error('NO_PENDING_ACTION', 'No original action is awaiting retry.');
    this.running = true;
    try {
      ...
      if (result.data.candidate.id !== p.body.candidateId || result.data.sessionId !== p.body.sessionId) throw new Api3Error('IDENTITY_MISMATCH', 'The write receipt did not match the original person and session.', '', true);
