...
export function RoleStandards({ data, inspect }: { data: Demo; inspect: (criterion?:string) => void }) {
  ...
  return <section className="eb-panel r5-standards-overview"><div className="eb-heading"><h2>One shared standard, ten reviewable judgments</h2><button className="eb-action" onClick={()=>inspect()}>Read the rubric</button></div><p className="eb-muted">Fixed assessment weights · not candidate scores.</p><div className="r5-weight-track" aria-label="Ten equal-weight assessment standards">{data.rubric.criteria.map(c=><button className={`r5-weight-cell r5-skill-${skillClass(c.requirementId)}`} key={c.id} onClick={()=>inspect(c.id)} aria-label={`${c.id} · ${c.maxScore} points · open rubric`}><span>{c.id}</span><small>{c.maxScore}</small></button>)}</div><div className="r5-skill-cards">{data.rubric.requirements.map(r=><section className={`r5-skill-card r5-skill-${skillClass(r.id)}`} key={r.id}><header><small>{data.rubric.criteria.filter(c=>c.requirementId===r.id).length} standards</small><strong className="r5-skill-weight">{r.maxScore}<small>%</small></strong></header><h3>{r.title}</h3><details><summary>What this role needs</summary><p>{r.statement}</p></details></section>)}</div></section>;
}
