...
function CandidateSession({ data, controller, page, go }: Props) {
  ...
  return <>
    {page === 'history' && <WorkflowStrip status={workflow.isTerminal ? 'closed' : task.status} started={draft.started} version={latest?.submission.submissionVersion ?? 1}/>}
    {page === 'workspace' && <LegacyDrafts currentKey={key}/>}{storageError && <p className="eb-feedback" role="alert">{storageError}</p>}
    {page === 'application' && <><Heading eyebrow={`${data.company.name} · ${data.job.title}`} title="Your application materials"><button className="eb-action" onClick={()=>go('tasks')}>View task status</button></Heading><p className="r5-intro">{candidate.background}</p><ApplicationMaterials data={data} base={controller.base}/><details className="eb-panel"><summary>Review full JD · {data.job.jd.requirements.length} requirements</summary><JobRequirements data={data} base={controller.base}/></details></>}
    {page === 'tasks' && <><Heading title="Your targeted task"/>{!sent ? <section className="eb-panel"><h2>No supplementary task requested</h2><p>Wait for a specific request from the hiring team, then refresh the shared case. A candidate with sufficient material can be compared directly.</p></section> : <><section className="eb-panel"><span className="r5-state">{workflow.isTerminal ? 'closed' : task.status.replaceAll('_', ' ')}</span><TaskBrief name={candidate.name} title={task.title} target={task.targetRequirementId} reason={task.gapReason} instructions={task.instructions} minutes={task.timeboxMinutes}/>{version ? <div className="eb-actions">{draft.started ? <button className="eb-action primary" onClick={() => go('workspace')}>Continue V{version} draft</button> : <DraftStartActions version={version} disabled={disabled} start={start}/>}</div> : <p>{workflow.isTerminal ? 'Evidence review is complete. No further submission is available.' : 'Your public snapshot is awaiting human review. V2 is not yet available.'}</p>}</section>{latest && <ReviewNote version={latest}/>}<details className="eb-panel"><summary>Task-provided resources</summary><p>Shared business data, not evidence authored by the candidate.</p><ResourceList resources={data.dataset.resources.filter(r=>task.targetRequirementId && data.taskTemplates[task.targetRequirementId].resourceIds.includes(r.id))}/></details></>}</>}
    {page === 'workspace' && <><Heading eyebrow={sent ? `${skill} · V${version ?? latest?.submission.submissionVersion ?? 1}` : undefined} title="Investigation workspace"/>{!sent || !version || !draft.started ? <section className="eb-panel"><p>{workflow.isTerminal ? 'This task is closed. Your previous versions remain available.' : latest ? 'Your formal snapshot is saved on the server. Read its public work and feedback.' : 'Open the task and start an available draft to edit.'}</p>{version && !draft.started ? <div className="eb-actions"><DraftStartActions version={version} disabled={disabled} start={start}/></div> : <button className="eb-action primary" onClick={() => go(latest ? 'history' : 'tasks')}>{latest ? 'View submitted work' : 'View task and work'}</button>}{latest && <ReviewNote version={latest}/>}</section> : <>
      <section className="eb-panel ia-brief-summary"><details><summary>Task brief & guidance</summary><p className="eb-preserve">{task.instructions}</p><p>{templateHint(task.targetRequirementId)}</p></details><small>{draft.savedAt ? `Local draft updated ${new Date(draft.savedAt).toLocaleString()}` : 'Local draft'} · {candidate.name} · V{version}</small>{version === 2 && <p>V1 stays unchanged. This draft answers the specific feedback below.</p>}</section>
