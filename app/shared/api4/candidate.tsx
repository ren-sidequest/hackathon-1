...
import { TaskBrief } from './guidance';
...
function CandidateSession({ data, controller, page, go }: Props) {
  ...
  return <>
    ...
    <LegacyDrafts currentKey={key}/>{storageError && <p className="eb-feedback" role="alert">{storageError}</p>}
    {page === 'application' && <><Heading eyebrow={`${data.company.name} · ${data.job.title}`} title="Your application materials"/><div className="r5-two"><SpotlightCard className="eb-panel"><h2>{candidate.name}</h2><p>{candidate.background}</p><p>Supplied fictional CV and labelled synthetic project companions. These are not newly uploaded files. Demo identity is separate from editing access.</p></SpotlightCard><StarBorder className="eb-panel"><h2>Only supplement what is needed</h2><p>{sent ? `A ${skill} task has been requested.` : 'No supplementary task has been requested. Your application can be compared using the available evidence.'}</p><p>Receiving a task is not rejection. Having no task is not an offer.</p><button className="eb-action primary" onClick={() => go('tasks')}>View task status</button></StarBorder></div><ApplicationMaterials data={data} base={controller.base}/><details className="eb-panel"><summary>Review full JD · {data.job.jd.requirements.length} requirements</summary><JobRequirements data={data} base={controller.base}/></details></>}
    {page === 'tasks' && <><Heading title="Your targeted task"/>{!sent ? <section className="eb-panel"><h2>No supplementary task requested</h2><p>Wait for a specific request from the hiring team, then refresh the shared case. A candidate with sufficient material can be compared directly.</p></section> : <><section className="eb-panel"><span className="r5-state">{workflow.isTerminal ? 'closed' : task.status.replaceAll('_', ' ')}</span><TaskBrief name={candidate.name} title={task.title} target={task.targetRequirementId} reason={task.gapReason} instructions={task.instructions} minutes={task.timeboxMinutes}/>{version ? <div className="eb-actions">{draft.started ? <button className="eb-action primary" onClick={() => go('workspace')}>Continue V{version} draft</button> : <><button className="eb-action primary" disabled={disabled} onClick={() => start()}>Start V{version} draft</button>{version === 2 && <button className="eb-action" disabled={disabled} onClick={() => start(true)}>Copy V1 public work into V2</button>}</>}</div> : <p>{workflow.isTerminal ? 'Evidence review is complete. No further submission is available.' : 'Your public snapshot is awaiting human review. V2 is not yet available.'}</p>}</section>{latest && <ReviewNote version={latest}/>}<section className="eb-panel"><h2>Task-provided resources</h2><p>Shared business data, not evidence authored by the candidate.</p><ResourceList resources={data.dataset.resources.filter(r=>task.targetRequirementId && data.taskTemplates[task.targetRequirementId].resourceIds.includes(r.id))}/></section></>}</>}
    {page === 'workspace' && <><Heading eyebrow={sent ? `${skill} · V${version ?? latest?.submission.submissionVersion ?? 1}` : undefined} title="Investigation workspace"/>{!sent || !version || !draft.started ? <section className="eb-panel"><p>{workflow.isTerminal ? 'This task is closed. Your previous versions remain available.' : latest ? 'Your formal snapshot is saved on the server. Read its public work and feedback.' : 'Open the task and start an available draft to edit.'}</p>{version && !draft.started ? <button className="eb-action primary" disabled={disabled} onClick={()=>start()}>Start V{version} draft</button> : <button className="eb-action primary" onClick={() => go(latest ? 'history' : 'tasks')}>{latest ? 'View submitted work' : 'View task and work'}</button>}{latest && <><ReviewNote version={latest}/><WorkSnapshot data={data} version={latest}/></>}</section> : <>
      ...
      {version === 2 && latest && <ReviewNote version={latest}/>}<nav className="guide-local-nav" aria-label="Work area shortcuts"><button className="eb-action" onClick={()=>document.getElementById('guide-resources')?.scrollIntoView({block:'start'})}>1 · Explore resources</button><button className="eb-action" onClick={()=>document.getElementById('guide-investigation')?.scrollIntoView({block:'start'})}>2 · Record your reasoning</button><button className="eb-action" onClick={()=>document.getElementById('guide-work-sample')?.scrollIntoView({block:'start'})}>3 · Check & submit</button></nav><Investigation data={data} draft={draft} update={update} record={record} disabled={disabled}/>
      <section id="guide-work-sample" className="eb-panel"><h2>Public work sample</h2><div className="guide-checklist" aria-label="Submission readiness"><span>{draft.summary.trim()?'✓':'○'} Summary {draft.summary.trim()?'added':'needed'}</span><span>{draft.findings.length} public cards · optional</span><span>Private notes excluded</span></div><label className="eb-field">Executive summary<textarea aria-label="Executive summary" maxLength={8000} disabled={disabled} value={draft.summary} onChange={e => update(d => ({ ...d, summary: e.target.value }))}/></label><small>{draft.summary.length}/8,000 characters · {draft.findings.length}/40 cards · {draft.events.length}/100 reported events</small>{draft.events.length >= 99 && <p>Process log is full. Existing events are preserved; one final slot is reserved for submission.</p>}<p>Weak or incomplete work may still be submitted. Private notes are excluded. Only saved public snapshots can be exported.</p><button className="eb-action primary" disabled={disabled} onClick={() => { setSubmitError(''); setConfirm(true); }}>Submit V{version}</button></section>
    </>}</>}
    {page === 'history' && <><Heading title="Work and public feedback"/>{data.versions.length ? <History key={latest!.submission.submissionId} data={data}/> : <section className="eb-panel"><h2>No formal work snapshot yet</h2><p>Your drafts are separate from submitted work.</p><button className="eb-action primary" onClick={() => go('tasks')}>View task status</button></section>}{version === 2 && <button className="eb-action primary" onClick={() => go('tasks')}>Read feedback and prepare V2</button>}{workflow.isTerminal && <p className="r5-notice">Task closed · no V3. Evidence confirmation is not a hiring decision.</p>}</>}
...
function Investigation({ data, draft, update, record, disabled }: { data: Demo; draft: CandidateDraft; update: (fn: (d: CandidateDraft) => CandidateDraft) => void; record: (title: string) => void; disabled: boolean }) {
  ...
  return <div className="eb-workbench r5-workbench"><section id="guide-resources" className="eb-panel"><h2>Data & resources</h2><small>Task-provided · {data.dataset.version}</small><ResourceList resources={data.dataset.resources} onEvent={record} onCreateFinding={!disabled && draft.findings.length < 40 ? setEditing : undefined}/></section><section className="eb-panel"><div className="eb-tabs" role="tablist" aria-label="Investigation tools">{['Explore', 'SQL', 'Data analysis', 'Private notebook'].map(t => <button className="eb-action" role="tab" key={t} aria-selected={tab === t} onClick={() => { setTab(t); record(`Opened ${t}`); }}>{t}</button>)}</div>
    ...
    </section><section id="guide-investigation" className="eb-panel eb-board"><h2>Investigation board</h2><details><summary>How to build your work sample</summary><p>Add a finding, name the supporting source and explain your reasoning. Keep hypotheses separate from observations. Describe missing data or a testable next step when useful.</p><p>Only your public cards and summary are submitted. You can submit incomplete work; explain its limitations.</p></details><small>{draft.findings.length}/40 cards · Public when submitted</small>{sections.map(section => <section key={section}><div className="eb-board-heading"><h3>{section}</h3><button className="eb-action" aria-label={`Add ${section}`} disabled={disabled || draft.findings.length >= 40} onClick={() => setEditing({ id: crypto.randomUUID(), section, title: '', detail: '', source: '', confidence: 'Medium' })}>+</button></div>{draft.findings.filter(f => f.section === section).map(f => <SpotlightCard as="article" key={f.id} className="eb-finding"><h4>{f.title}</h4><p className="eb-preserve">{f.detail}</p><small>{(data.dataset.resources.find(r => r.id === f.source)?.name ?? f.source) || 'No source'} · Self-confidence {f.confidence}</small><button className="eb-action" aria-label={`Edit ${f.title}`} disabled={disabled} onClick={() => setEditing(f)}>Edit</button></SpotlightCard>)}{!draft.findings.some(f => f.section === section) && <p className="eb-muted">No cards yet · optional</p>}</section>)}</section>
...
function History({ data }: { data: Demo }) {
  ...
  return <><label className="eb-field">Submission version<GlideSelect ariaLabel="Submission version" value={String(selected)} onChange={value => setSelected(Number(value) as 1 | 2)} options={data.versions.map(v => ({value:String(v.submission.submissionVersion),label:`V${v.submission.submissionVersion}${v.submission.submissionId === data.versions.at(-1)?.submission.submissionId ? ' · current' : ' · history'}`}))}/></label><ReviewNote version={version}/>{data.versions.length === 2 && <VersionComparison before={data.versions[0].submission} after={data.versions[1].submission}/>}<WorkSnapshot data={data} version={version}/></>;
}
