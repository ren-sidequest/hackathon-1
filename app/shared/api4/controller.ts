...
export function useApi4(role: 'hr' | 'candidate', candidateId: CandidateId | null, needsSelection = false): Api4Controller {
  ...
  const execute = async (action: () => ReturnType<Api4Client['retry']>, owner: unknown, session: unknown, isAnalysis = false, path = '', stage?: unknown) => {
    ...
    try {
      ...
      const saved = path === '/rehearsal/restart' ? 'Rehearsal restarted · previous shared work archived.' : path === '/submission' ? 'Work submitted · waiting for human review.' : path === '/task/send' ? 'Task sent · available in the candidate workspace.' : path === '/review' ? 'Evidence review saved · public feedback is available.' : path === '/assessment' ? 'Human assessment saved · server scores updated.' : path === '/shortlist' ? 'Shortlist decision saved.' : 'Action saved.';
