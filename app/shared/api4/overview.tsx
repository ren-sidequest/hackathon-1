...
export function StandardsOverview({ data, compare }: { data: Demo; compare: () => void }) {
  ...
  return <section className="eb-panel r5-standards-overview" aria-labelledby="standards-overview-title">
    ...
    <footer className="r5-standards-footer"><div><ul className="r5-role-constraints" aria-label="Shared role constraints"><li>{data.job.location}</li><li>{data.job.employmentType}</li><li>{data.job.experienceLevel}</li></ul><small>{data.company.businessDescription}</small></div><button className="eb-action primary" onClick={compare}>Compare four candidates</button></footer>
...
export function ReviewDashboard({ comparison, select }: { comparison: Comparison; select: (id: CandidateId, page?: string) => void }) {
  ...
  const queue = rows.flatMap(row => {
    ...
    if (!pending && row.gapSuggestions.length === 0) return [];
  ...
  return <div className="r5-dashboard">
    <SpotlightCard className="eb-panel r5-coverage" aria-label="Evidence coverage overview"><div className="eb-heading"><div><small>APPLICATION MATERIALS</small><h2>Evidence coverage</h2></div><span className="r5-state">{rows.length} profiles</span></div><p className="eb-muted">Criteria with numerical evidence out of ten · NE is reviewed but not numerically covered.</p>
