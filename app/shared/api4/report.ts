import type { AssessmentRecord, Demo, Stage } from '../api4-types';
...
export function assessmentReport(data: Demo, stage: Stage, selected?: AssessmentRecord | null, base = '/gateway'): string {
  ...
  if (stage === 'application_review') {
    lines.push('', '## Full JD alignment · separate from the analytical score', `JD version: ${data.jdVersion}`, `Original JD: ${materialUrl(base, data.job.jd.source.downloadUrl) ?? 'unavailable'}`);
    for (const requirement of data.job.jd.requirements) {
      const match = data.application.jdAlignment.find(a => a.jdRequirementId === requirement.id);
      lines.push(`### ${requirement.statement}`, `${requirement.category} · JD page ${requirement.page}`, match?.statuses.map(s=>s.replaceAll('_', ' ')).join(' · ') ?? 'Not reviewed', match?.summary ?? '', `Unknown / check next: ${match?.remainingUnknowns ?? 'Not reviewed'}`);
      for (const ref of match?.sourceRefs ?? []) if (resolveSourceRef(data.candidate.id, data.application, ref)) lines.push(`Source: ${ref.sourceId} · ${ref.location}`, ref.quote);
    }
    for (const source of data.application.sources) if (source.provenance) lines.push(`Material: ${source.sourceId}`, source.provenance.disclosure, `Pages: ${source.provenance.pageNumbers.join(', ') || 'Companion text'}`, `Displayed text SHA-256: ${source.provenance.sha256}`, source.provenance.downloadUrl ? `Original PDF: ${materialUrl(base, source.provenance.downloadUrl) ?? 'unavailable'}` : 'Synthetic work sample · not a formal task submission');
  }
