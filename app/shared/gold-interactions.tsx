...
type CardProps = HTMLAttributes<HTMLElement> & { as?: 'div' | 'section' };
...
export function StarBorder({ as: Tag = 'section', className = '', children, ...props }: CardProps) {
  return <Tag {...props} className={`eb-star-border ${className}`}><span className="eb-star-trail" aria-hidden="true" />{children}</Tag>;
}
