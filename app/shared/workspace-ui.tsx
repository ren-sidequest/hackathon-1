...
export function DataOverview({ dataset, onEvent }: { dataset: Demo['dataset']; onEvent?: (title: string) => void }) {
  ...
  return <section className="eb-data-overview" aria-label="Dataset overview"><div className="eb-heading"><div><h2>Website performance overview</h2><small>Previous 4 weeks → Last 4 weeks · {dataset.version}</small></div></div><div className="eb-metrics">{[
    ...
    <div className="eb-data-controls"><label>Channel<GlideSelect ariaLabel="Filter channel" value={channel} onChange={value => { setChannel(value); onEvent?.(`Filtered ${value}`); }} options={[{value:'All channels',label:'All channels'}, ...dataset.channels.map(c => ({value:c.channel,label:c.channel}))]}/></label><label>Sort rows<GlideSelect ariaLabel="Sort channel rows" value={sort} onChange={setSort} options={[{value:'default',label:'Dataset order'},{value:'conversion',label:'Conversion · lowest first'},{value:'traffic',label:'Sessions · highest first'}]}/></label></div>
