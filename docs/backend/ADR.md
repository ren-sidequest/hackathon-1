...
日期：2026-09-19。范围：单机合成案例、同一任务 V1 + 最多一次 V2 补交、每版最多一次人工审核；决策用于本轮本地实现，非生产架构或真人团队审批记录。

依据：用户确认的完整参赛方案修订 3，第 1、7、8、9、10、17 节，以及仓库[产品蓝图](../product/EvidenceBridge_PRODUCT_BLUEPRINT.md)与[交互基线](../product/EvidenceBridge_BASELINE.md)。本轮取代单轮限制，仅 V1 More 开放一次 V2；旧前端无限补交和重开审核行为仍不采用。先行决策见 [REVISION_PLAN](REVISION_PLAN.md)，未引入上传、岗位 CRUD 或账号平台。
...
每版正式提交一次性保存快照和服务端时间，绑定 `taskId`、`submissionId`、`submissionVersion`、`datasetVersion`、会话代次、前版 ID/指纹与本版内容指纹。V1 前版字段为 null；V2 明确绑定 V1，不改变 session/task/dataset。指纹来自确定性序列化的公开提交字段；引文定位使用保存文本本身，不另行改写。V2 追加新快照，不覆盖 V1；reset 只用于另一场演示，不用来模拟补交。

四类概念分开持久化：

| 概念 | 语义 | 关键约束 |
| --- | --- | --- |
| 任务进度 | 未发送、已发送、已提交、等待补交、已审核 | V1 More 才进入 awaiting_revision；工作中草稿不等于正式版本 |
| AI 分析 | 未开始、运行中、成功、失败 | 成功只表示观察提取完成 |
| 人工审核结果 | Confirm / Needs More Evidence / Evidence Still Insufficient | 每版最多一次；仅 V1 More 开放 V2，其余为任务终局；不是录用结论 |
| 岗位证据支持 | Supported / Uncertain / Verified through targeted task | Confirm 仅更新 Business Problem Solving；另外两项保持初始支持情况 |

两种不足结果保留 `Uncertain`、审核备注和剩余未知。只有 V1 More 开放一次 V2，本地草稿按会话/任务/拟提交版本隔离；V2 新建空分析和空审核，V1 结果只读保留。AI 失败仍保留作品供人审，人工审核可直接基于真实提交，不依赖模型成功。正式字段名与完整错误响应以后端接口合同和可执行 schema 为准，不在本文重复维护第二份合同。

## ADR-03：短事务、幂等与 reset 代次

**决定。** 每次业务写入绑定当前随机会话 ID；reset 原子替换为新会话。幂等键按会话、方法和路径隔离，并绑定规范化请求摘要：同键同请求返回原结果；同键异请求冲突。不同键重复提交或审核仍遵守最多两版、每版一次以及前版绑定约束，避免通过换键绕过业务顺序。旧 key 可返回 V1 历史收据，但新 key 对历史版的分析/审核返回 STALE_SUBMISSION；客户端一律重新 GET 取得当前投影。任务状态、正式对象及幂等结果在同一事务内提交。

AI 外部调用放在事务外：短事务登记分析 attempt，再调用模型，返回后短事务核验会话、当前提交 ID、内容指纹、attempt、仍运行且未审核。人审时运行中的分析冻结为 failed/AI_REVIEW_CLOSED，202 收据同步冻结；迟到结果报 STALE_ANALYSIS，不改已审 V1 或 V2。reset 后失效结果也不写入新会话。运行中请求的重复行为必须有确定状态响应；失败允许显式新尝试，但不删除作品。进程启动将中断的分析标记为失败/可重试，而非永久停留在运行中。

**代价。** 这是单案例的两版记录和冲突控制，不是通用事件溯源、任意历史检索或分布式调度。数据库文件与 SQLite 辅助文件是本机运行状态，排除 Git；刷新和进程重启由相同文件恢复，reset 使用新会话让旧浏览器提交明确失效。
...
模型适配层隔离提供者协议、服务端凭据、输入/输出大小、超时、有限重试与结构校验。模型失败返回稳定错误，保留快照，避免把网络、格式或引文错误伪装成成功观察。分析结果始终绑定所属版提交与指纹。V2 只分析 V2，V1 输出在 V1 历史中仍有效，不用于 V2；reset 后旧会话结果失效。同义改写不要求结论变化。
...
**验收。** API 测试客户端验证发送→唯一测试句提交→读取相同快照→引用定位→人工审核仅影响目标要求→重读→重置重演，同时验证 V1 More→同任务 V2→终局、两版只读引用与评论、非法第三版/二版 More、审核与分析竞争，以及顺序错误、重复请求、过期会话/提交、私有字段、模型失败、重启恢复。现有前端回归另行运行。阶段结论区分“后端验收完成”与“双端 UI 联调待小傅接入”，不以 API 成功替代 UI 已接通。

## ADR-06：有限两版合同与显式新库

**状态：修订 3 已确认。** API schema 升为 2.0，沿用六个端点。GET 保留当前 submission/analysis/review，增加最多两条 versions 与 workflow，不增加历史修改端点。remainingSubmissions 仅是容量；动作由 canSubmit/canResubmit/nextSubmissionVersion/allowedReviewDecisions/isTerminal 给出。V1 终局即使容量为 1，也关闭补交。

**存储决定。** SQLite 文件格式升为 2；submission_version 与 session 的唯一约束、不可变快照及启动关联校验共同约束两版链。默认 `var/evidencebridge-v2.sqlite`，旧 1.0 数据库检测后停止启动、保持文件和记录，要求新的 DATABASE_PATH；不自动迁移、不静默 reset。正常 schema 2.0 同库重启恢复等待补交、当前版及两版历史；reset 明确清除该演示的全部版本。

**取舍。** 覆盖 V1 会破坏原引文与人审对象，故不采用；无限补交/复杂 diff/通用版本平台超出范围。旧演示库小且无生产迁移需求，采用新路径而非增加未经验证的转换逻辑。代价是旧客户端需更新至 2.0，旧 .env 需显式选择新库，UI 需维护版本选择与按版草稿。真实模型实验、UI 联调及远端发布仍各自验收，不由 schema 升级代替。

## 依赖来源核验（2026-09-19，原技术选型记录）
